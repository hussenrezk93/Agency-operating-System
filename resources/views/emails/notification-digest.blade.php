...
<head>
    <meta charset="utf-8">
    <title>{{ __('agencyos.notifications.email.digest_subject', ['count' => $notifications->count()]) }}</title>
</head>
    ...
    <table role="presentation" width="100%" style="max-width: 560px; margin: 0 auto; background: #ffffff; border-radius: 8px; overflow: hidden">
        ...
        <tr>
            <td style="padding: 24px">
                <h1 style="font-size: 18px; margin: 0 0 16px">{{ __('agencyos.notifications.email.digest_heading', ['count' => $notifications->count()]) }}</h1>
